...
import { Plus, Save, Zap, ChevronRight, Bus, Loader2, Search, X, Maximize2, Minimize2, Clock, ArrowDownWideNarrow, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
const RouteStudio: React.FC = () => {
    ...
    const timeToSeconds = (t: string) => {
        if (typeof t !== 'string') {
            throw new Error(`Invalid time input: expected string, got ${typeof t}`);
        }
        ...
        if (segments.length < 2 || segments.length > 3) {
            throw new Error(`Invalid time format: "${trimmed}". Expected "hh:mm" or "hh:mm:ss"`);
        }

        const nums = segments.map(s => {
            const n = Number(s);
            if (!Number.isFinite(n) || s.trim() === '') {
                throw new Error(`Invalid numeric segment in time: "${s}" in "${trimmed}"`);
            }
            return n;
        });
        ...
        if (h < 0) throw new Error(`Invalid hours: ${h}. Must be non-negative.`);
        if (m < 0 || m > 59) throw new Error(`Invalid minutes: ${m}. Must be between 0 and 59.`);
        if (s < 0 || s > 59) throw new Error(`Invalid seconds: ${s}. Must be between 0 and 59.`);
    ...
    const togglePersistentRoute = async (routeId: number) => {
        ...
        if (!persistentRouteShapes[routeId]) {
            try {
                ...
                console.error("Failed to load persistent route shape:", e);
            }
        }
    };
    ...
    useEffect(() => {
        const pRoutes = persistentRouteIds.map(id => ({
            ...
            positions: persistentRouteShapes[id] || [],
            isFocused: false
        ...
        setMapLayers(prev => ({
            ...prev,
            routes: [
                ...(selectedRoute ? [{ id: selectedRoute.id, color: selectedRoute.color, positions: shapePoints.map(p => [p.lat, p.lon] as [number, number]), isFocused: true }] : []),
                ...pRoutes
            ],
