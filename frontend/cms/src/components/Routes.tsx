...
const RouteStudio: React.FC = () => {
    ...
    const handleShapePointInsert = useCallback(async (index: number, latlng: { lat: number, lng: number }) => {
        ...
        if (autoRoute && index > 0 && index < shapePoints.length) {
            setStatus({ message: 'System tracing road path. Please wait.', type: 'loading' });
            try {
                ...
                if (res.data.routes && res.data.routes[0]) {
                    ...
                    setStatus({ message: 'Path updated. Save to synchronize.', type: 'success' });
                    setTimeout(() => setStatus(null), 2000);
                    return;
                }
            } catch (e) { setStatus({ message: 'Path tracing failed.', type: 'error' }); }
        }
        const newPoints = [...shapePoints];
    ...
    const snapStopsToPath = useCallback(() => {
        ...
        setStatus({ message: 'System aligning stops. Please wait.', type: 'loading' });
        ...
        setIsDirty(true);
        setStatus({ message: 'Stops aligned. Save to synchronize.', type: 'success' });
        setTimeout(() => setStatus(null), 2000);
    }, [shapePoints, assignedStops, setStatus]);

    const saveChanges = useCallback(async (isAuto = false) => {
        if (!selectedRoute) return;
        if (!isAuto) setStatus({ message: 'Saving changes to server. Please wait.', type: 'loading' });
        try {
            ...
            setIsDirty(false);
            if (!isAuto) { setStatus({ message: 'All changes saved successfully.', type: 'success' }); setTimeout(() => setStatus(null), 2000); }
            await refreshAllData();
            ...
            if (autoRoute && shapePoints.length > 0) {
                ...
                try {
                    ...
                    if (res.data.routes && res.data.routes[0]) {
                        ...
                        setStatus({ message: 'Path extended. Save to synchronize.', type: 'success' });
                        setTimeout(() => setStatus(null), 2000);
                        return;
                    }
    ...
    }, [activeSection, selectedRoute, shapePoints, autoRoute, pushToHistory, setStatus, quickMode, setQuickMode]);
    ...
    return (
        <div className="absolute inset-0 flex overflow-visible pointer-events-none font-bold">
            <motion.div 
                ...
                style={{ width: 400 }}
            >
                <SidebarHeader title="Routes" Icon={Bus} actions={<button onClick={handleAddNew} className="p-2 bg-system-blue text-white rounded-lg shadow-lg hover:scale-105 transition-all" title="Add a new route"><Plus size={18} /></button>} />
                ...
                >
                    <div className="p-4 pb-3 flex items-center justify-between shrink-0 cursor-move border-b border-black/[0.03]">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0"><h2 className="text-sm font-black tracking-tight truncate leading-none mb-0.5">{selectedRoute.short_name || 'New Route'}</h2><p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest truncate">Route Editor</p></div>
                        </div>
                        <div className="flex items-center gap-0.5">
                            <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-1.5 hover:bg-black/5 rounded-full text-zinc-400">{isCollapsed ? <Maximize2 size={14}/> : <Minimize2 size={14}/>}</button>
                            <button onClick={() => setSelectedRoute(null)} className="p-1.5 hover:bg-black/5 rounded-full text-zinc-400 transition-all hover:rotate-90"><X size={16}/></button>
                        </div>
                    </div>
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 pt-2 custom-scrollbar">
                                ...
                                {activeSection === 'sequence' && (
                                    <div className="space-y-4 animate-in fade-in duration-300">
                                        <button onClick={snapStopsToPath} className="w-full py-2.5 bg-white border border-zinc-200 text-zinc-600 rounded-xl font-black text-[9px] uppercase hover:bg-zinc-50">Align stops to path</button>
                                        <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1.5 custom-scrollbar">
                                            <Reorder.Group axis="y" values={assignedStops} onReorder={(newOrder) => { setAssignedStops(newOrder); setIsDirty(true); }} className="space-y-1.5">
                                                {assignedStops.map((rs, i) => (
                                                    <Reorder.Item key={rs.stop_id} value={rs} className="flex items-center gap-3 p-2.5 bg-white/50 border border-zinc-100 rounded-xl cursor-grab hover:bg-white hover:shadow-sm transition-all group">
                                                        <div className="w-5 h-5 rounded-full bg-zinc-50 flex items-center justify-center text-[8px] font-black text-zinc-400 group-hover:bg-system-blue group-hover:text-white">{i+1}</div>
                                                        <div className="flex-1 font-black text-[10px] truncate uppercase text-zinc-900">{rs.stop?.name}</div>
                                                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            <button onClick={(e) => { e.stopPropagation(); setSelectedEntityId(rs.stop_id); navigate('/stops'); }} className="p-1 text-orange-400 hover:text-orange-600"><MapPin size={12} /></button>
                                                            <button onClick={() => { setAssignedStops(assignedStops.filter((_, idx) => idx !== i)); setIsDirty(true); }} className="p-1 text-red-400 hover:text-red-600"><X size={14}/></button>
                                                        </div>
                                                    </Reorder.Item>
                                                ))}
                                            </Reorder.Group>
