...
const Stops: React.FC = () => {
    ...
    const handleRouteHighlight = async (routeId: number, isPersistent: boolean) => {
        if (isPersistent) {
            setSelectedStopRouteIds(prev => [...new Set([...prev, routeId])]);
        } else {
    ...
    useEffect(() => {
        ...
        setMapLayers(prev => ({
            ...prev,
            routes: selectedStopRouteIds.map(rid => ({
    ...
    }, [stops, routeShapes, formData, routes, hoveredRouteIds, selectedStopRouteIds, hoveredEntityId, focusType, setMapLayers]);
