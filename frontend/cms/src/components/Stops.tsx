...
import { MapPin, Plus, Trash2, Search, Filter, Loader2, CheckCircle2, ChevronLeft, ChevronRight, Save, X, Layers } from 'lucide-react';
import api from '../api';
...
const Stops: React.FC = () => {
    ...
    // Form State
    const [formData, setFormData] = useState<{ name: string, lat: string | number, lon: string | number }>({ name: '', lat: '', lon: '' });
    ...
    const handleSave = useCallback(async (e?: React.FormEvent) => {
        ...
        const payload = { 
            name: formData.name, 
            lat: typeof formData.lat === 'string' ? parseFloat(formData.lat) : formData.lat, 
            lon: typeof formData.lon === 'string' ? parseFloat(formData.lon) : formData.lon 
        };
        try {
    ...
    useEffect(() => {
        setMapLayers(prev => ({
            ...
            focusedPoints: editingId ? [[Number(formData.lat), Number(formData.lon)]] : [],
            activeShape: []
        }));
    ...
    return (
        <div className="flex h-full bg-white shadow-2xl relative z-20 overflow-hidden" style={{ width: sidebarOpen ? '450px' : '0', transition: 'width 0.3s ease' }}>
            <div className="p-6 border-b border-black/5 flex items-center justify-between shrink-0">
                ...
                <div className="flex gap-2">
                    <button onClick={() => setAddFormOpen(!addFormOpen)} className={`p-2 rounded-lg transition-colors ${addFormOpen ? 'bg-system-blue text-white' : 'hover:bg-black/5 text-system-gray'}`} title="Toggle New Stop Form"><Plus size={20}/></button>
            ...
            <div className={`transition-all duration-500 overflow-hidden shrink-0 ${addFormOpen ? 'max-h-[400px]' : 'max-h-0 opacity-0 pointer-events-none'}`}>
            ...
            <div className="p-4 border-b border-black/5 bg-white sticky top-0 z-10 shrink-0 font-bold">
                ...
                {focusedRouteId && (
                    <div className="mt-3 flex items-center justify-between">
                        <Badge className="bg-system-blue/10 text-system-blue border-none px-2 py-1 text-[10px] font-black flex items-center gap-1.5 uppercase"><Filter size={10}/> Filtering by Line {routes.find(r=>r.id===focusedRouteId)?.short_name}</Badge>
            ...
            <div className="p-4 border-t border-black/5 bg-white shrink-0 font-bold">
