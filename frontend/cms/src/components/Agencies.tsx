                ...
                >
                    <div className="p-4 pb-3 flex items-center justify-between shrink-0 cursor-move border-b border-black/[0.03]">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-system-blue text-white shadow-lg shrink-0"><Landmark size={16} /></div>
                            <div className="min-w-0">
                                <h2 className="text-sm font-black tracking-tight truncate leading-none mb-0.5">{formData.name || 'New Agency'}</h2>
                                <p className="text-[8px] font-black text-system-gray uppercase tracking-widest truncate opacity-60">Profile</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-0.5">
                            ...
                            <button onClick={() => setSelectedAgency(null)} className="p-1.5 hover:bg-black/5 rounded-full text-system-gray transition-all hover:rotate-90"><X size={16}/></button>
                        </div>
                    </div>
                        ...
                        <>
                            <div className="flex-1 overflow-y-auto p-4 pt-2 custom-scrollbar">
                                <form onSubmit={handleSave} className="space-y-4">
                                    <div><label className="text-[8px] font-black uppercase mb-1 block text-system-gray opacity-60 tracking-widest">Formal Name</label>
                                    <input className="hig-input text-[11px] font-bold py-1.5" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required /></div>
                                    <div><label className="text-[8px] font-black uppercase mb-1 block text-system-gray opacity-60 tracking-widest">Website</label>
                                    <div className="relative"><input className="hig-input text-[11px] font-bold py-1.5 pr-8" value={formData.url} onChange={e => setFormData({...formData, url: e.target.value})} required /><ExternalLink size={12} className="absolute right-2.5 top-2.5 text-system-gray opacity-40" /></div></div>
                                    <div><label className="text-[8px] font-black uppercase mb-1 block text-system-gray opacity-60 tracking-widest">Timezone</label>
                                    <select className="hig-input text-[11px] font-bold py-1.5" value={formData.timezone} onChange={e => setFormData({...formData, timezone: e.target.value})} required>
                                        <option value="">Identify...</option><option value="Asia/Jakarta">Jakarta</option><option value="Asia/Makassar">Makassar</option>
                                    </select></div>
                                    <div className="grid grid-cols-2 gap-2 pt-2">
                                        <div className="bg-black/5 p-2 rounded-xl text-center"><div className="text-lg font-black text-black leading-none mb-0.5">{selectedAgency.id ? agencyStats[selectedAgency.id]?.routes || 0 : 0}</div><div className="text-[7px] font-black text-system-gray uppercase">Lines</div></div>
                                        <div className="bg-black/5 p-2 rounded-xl text-center"><div className="text-lg font-black text-black leading-none mb-0.5">{selectedAgency.id ? agencyStats[selectedAgency.id]?.stops || 0 : 0}</div><div className="text-[7px] font-black text-system-gray uppercase">Nodes</div></div>
                                    </div>
                                    {selectedAgency.id && (
                                        <button type="button" onClick={() => { if(window.confirm('Wipe record?')) api.delete(`/agencies/${selectedAgency.id}`).then(fetchInitialData).then(() => setSelectedAgency(null)); }} className="w-full py-2 text-[8px] font-black text-red-400 hover:text-red-600 uppercase tracking-widest">Terminate</button>
                                    )}
                                </form>
                            </div>
                            <div className="p-4 bg-white/50 backdrop-blur-md border-t border-black/5 rounded-b-[1.5rem] sticky bottom-0">
                                <button onClick={handleSave} disabled={!isDirty} className="w-full py-3.5 bg-system-blue text-white rounded-xl font-black text-[9px] shadow-xl shadow-system-blue/20 transition-all active:scale-95 tracking-widest uppercase">Sync Manifest</button>
                            </div>
                        </>
                    )}
                </motion.div>
