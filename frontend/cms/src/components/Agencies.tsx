        ...
        <div className="absolute inset-0 flex overflow-visible pointer-events-none font-bold">
            ...
            <motion.div initial={false} animate={{ x: sidebarOpen ? 0 : -320 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="flex flex-col h-full bg-white dark:bg-zinc-900 relative z-20 overflow-hidden text-black dark:text-white border-r border-zinc-200 dark:border-zinc-800 pointer-events-auto shadow-none" style={{ width: 320 }}>
                <SidebarHeader
                    ...
                    actions={<button onClick={handleAddNew} className="p-1.5 bg-blue-50 dark:bg-zinc-800 text-blue-600 rounded-sm hover:bg-blue-100 dark:hover:bg-zinc-700 transition-colors" title="Add Operator"><Plus size={18} /></button>}
                />
                <div className="p-4 px-6 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 shrink-0 flex gap-2 overflow-hidden">
                    ...
                    <button onClick={fetchInitialData} className="p-2 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-sm text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors shrink-0"><RotateCcw size={18} /></button>
                </div>
                <div className="flex-1 overflow-y-auto divide-y divide-zinc-50 dark:divide-zinc-800">
                    {filteredAgencies.map(agency => (
                        <div key={agency.id} className={`p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800 cursor-pointer transition-colors duration-75 group flex items-center justify-between ${selectedAgency?.id === agency.id ? 'bg-blue-50/50 dark:bg-blue-900/20 border-l-2 border-blue-600' : ''}`} onClick={() => handleSelectAgency(agency)}>
