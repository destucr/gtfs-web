                ...
                >
                    <div className="p-6 pb-4 flex items-center justify-between shrink-0 cursor-move">
                        <div className="flex items-center gap-4 flex-1 min-w-0">
                            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-system-blue text-white shadow-xl shadow-system-blue/20 shrink-0"><Landmark size={20} /></div>
                            <div className="min-w-0">
                                <h2 className="text-lg font-black tracking-tight truncate leading-none mb-1">{formData.name || 'New Agency'}</h2>
                                <p className="text-[9px] font-black text-system-gray uppercase tracking-[0.15em] truncate opacity-60">ID: {selectedAgency.id || 'RESERVED'}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => setSelectedAgency(null)} className="p-1.5 hover:bg-black/5 rounded-full text-system-gray transition-all hover:rotate-90"><X size={18}/></button>
                        </div>
                    </div>
                    ...
                    {!isCollapsed && (
                        <>
                            ...
                            <div className="p-6 bg-white/50 backdrop-blur-md border-t border-black/5 rounded-b-[2rem] sticky bottom-0">
                                <button onClick={handleSave} disabled={!isDirty} className="w-full py-4 bg-system-blue text-white rounded-2xl font-black text-[10px] shadow-2xl shadow-system-blue/30 transition-all disabled:opacity-30 active:scale-95 uppercase tracking-widest">Sync Operator Manifest</button>
                            </div>
                        </>
                    )}
                </motion.div>
