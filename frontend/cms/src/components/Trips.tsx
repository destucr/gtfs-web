        ...
        <div className="absolute inset-0 flex overflow-visible pointer-events-none font-bold">
            ...
            <motion.div initial={false} animate={{ x: sidebarOpen ? 0 : -320 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="flex flex-col h-full bg-white dark:bg-zinc-950 relative z-20 overflow-hidden text-black dark:text-white border-r border-zinc-200 dark:border-zinc-800 pointer-events-auto shadow-none" style={{ width: 320 }}>
                <SidebarHeader
                    ...
                    actions={<button onClick={handleAddNew} className="p-1.5 bg-blue-50 dark:bg-zinc-900 text-blue-600 rounded-sm hover:bg-blue-100 dark:hover:bg-zinc-800 transition-colors" title="Add a new binding"><Plus size={18} /></button>}
                />
                <div className="p-4 px-6 border-b border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0"><div className="relative"><Search size={14} className="hig-input-icon" /><input className="hig-input pl-8" placeholder="Search trips..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} /></div></div>
                <div className="flex-1 overflow-y-auto divide-y divide-zinc-50 dark:divide-zinc-800">
                    {filteredTrips.map(trip => (
                        <div key={trip.id} className={`p-4 hover:bg-zinc-50 dark:hover:bg-zinc-900 cursor-pointer transition-colors duration-75 group flex items-center justify-between ${selectedTrip?.id === trip.id ? 'bg-blue-50/50 dark:bg-blue-900/20 border-l-2 border-blue-600' : ''}`} onClick={() => handleSelectTrip(trip)}>
                            <div className="flex items-center gap-4 min-w-0">
                                <div className="w-2 h-10 rounded-full shrink-0" style={{ backgroundColor: `#${(trip.route?.color || 'ddd').replace('#', '')}` }}></div>
                                <div className="min-w-0"><div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 leading-none mb-1 truncate">{trip.route?.short_name || '??'}</div><div className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest truncate">{trip.headsign}</div></div>
                            </div>
                            <div className="flex items-center gap-1">
                                <button onClick={(e) => { e.stopPropagation(); if (window.confirm('Wipe binding?')) api.delete(`/trips/${trip.id}`).then(fetchInitialData); }} className="p-1.5 text-zinc-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-sm opacity-0 group-hover:opacity-100 transition-all duration-75"><Trash2 size={14} /></button>
                                <ChevronRight size={18} className={`text-zinc-300 transition-all duration-75 ${selectedTrip?.id === trip.id ? 'translate-x-1 text-blue-600 opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {selectedTrip && (
                <motion.div drag dragMomentum={false} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={`absolute top-6 z-[3000] w-[320px] bg-white dark:bg-zinc-900 rounded-[1.5rem] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] border border-black/5 dark:border-white/5 flex flex-col transition-all duration-500 pointer-events-auto ${quickMode && !isHovered ? 'opacity-20 pointer-events-none scale-95 blur-sm' : 'opacity-100'}`} style={{ right: 24, height: isCollapsed ? 'auto' : 'calc(100vh - 120px)' }} initial={{ opacity: 0, x: 20 }} animate={{ opacity: (quickMode && !isHovered ? 0.2 : 1), x: 0 }}>
                    ...
                    {!isCollapsed && (<><div className="flex-1 overflow-y-auto p-4 pt-2 custom-scrollbar dark:bg-zinc-900"><form onSubmit={handleSave} className="space-y-4"><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400 dark:text-zinc-500">Master Line</label><select className="hig-input text-[11px] font-bold py-1.5" value={formData.route_id} onChange={e => setFormData({ ...formData, route_id: e.target.value })} required><option value="">Select...</option>{routes.map(r => <option key={r.id} value={r.id}>{r.short_name} &mdash; {r.long_name}</option>)}</select></div><div className="grid grid-cols-2 gap-3"><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400 dark:text-zinc-500">Service ID</label><input className="hig-input text-[11px] font-bold py-1.5 uppercase" placeholder="e.g. DAILY" value={formData.service_id} onChange={e => setFormData({ ...formData, service_id: e.target.value })} required /></div><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400 dark:text-zinc-500">Direction</label><select className="hig-input text-[11px] font-bold py-1.5" value={formData.direction_id} onChange={e => setFormData({ ...formData, direction_id: e.target.value })} required><option value="0">0 - Outbound</option><option value="1">1 - Inbound</option></select></div></div>                                        <div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400 dark:text-zinc-500">Heading</label><input className="hig-input text-[11px] font-bold py-1.5" value={formData.headsign} onChange={e => setFormData({ ...formData, headsign: e.target.value })} required /></div>
                        <div>
                            <label className="text-[8px] font-black uppercase mb-1 block text-zinc-400 dark:text-zinc-500">Linked Path (Shape)</label>
                            <select
