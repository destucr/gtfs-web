...
const Trips: React.FC = () => {
    ...
    useEffect(() => {
        ...
        else if (selectedTrip) setStatus({ message: 'All changes saved.', type: 'info', isDirty: false });
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-zinc-50">
                    {filteredTrips.map(trip => (
                        <div key={trip.id} className={`p-4 hover:bg-zinc-50 cursor-pointer transition-all group flex items-center justify-between ${selectedTrip?.id === trip.id ? 'bg-system-blue/5 border-l-4 border-system-blue' : ''}`} onClick={() => handleSelectTrip(trip)}>
                            ...
                            <ChevronRight size={18} className={`text-zinc-300 transition-all ${selectedTrip?.id === trip.id ? 'translate-x-1 text-system-blue' : 'opacity-0 group-hover:opacity-100'}`} />
                        </div>
                    ))}
                </div>
            </motion.div>
            ...
            {selectedTrip && (
                <motion.div drag dragMomentum={false} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} className={`absolute top-6 z-[3000] w-[320px] bg-white/90 backdrop-blur-xl rounded-[1.5rem] shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] border border-black/5 flex flex-col transition-all duration-500 pointer-events-auto ${quickMode && !isHovered ? 'opacity-20 pointer-events-none scale-95 blur-sm' : 'opacity-100'}`} style={{ right: 24, height: isCollapsed ? 'auto' : 'calc(100vh - 120px)' }} initial={{ opacity: 0, x: 20 }} animate={{ opacity: (quickMode && !isHovered ? 0.2 : 1), x: 0 }}>
                    <div className="p-4 pb-3 flex items-center justify-between shrink-0 cursor-move border-b border-black/[0.03]">
                        <div className="flex items-center gap-3 flex-1 min-w-0"><div className="w-8 h-8 rounded-lg flex items-center justify-center bg-system-blue text-white shadow-lg shrink-0"><Database size={16} /></div><div className="min-w-0"><h2 className="text-sm font-black tracking-tight truncate leading-none mb-0.5">{formData.headsign || 'New'}</h2><p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest truncate">Trip Details</p></div></div>
                    ...
                    {!isCollapsed && (<><div className="flex-1 overflow-y-auto p-4 pt-2 custom-scrollbar"><form onSubmit={handleSave} className="space-y-4"><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400">Master Line</label><select className="hig-input text-[11px] font-bold py-1.5" value={formData.route_id} onChange={e => setFormData({...formData, route_id: e.target.value})} required><option value="">Select...</option>{routes.map(r => <option key={r.id} value={r.id}>{r.short_name} &mdash; {r.long_name}</option>)}</select></div><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400">Signature</label><input className="hig-input text-[11px] font-bold py-1.5" placeholder="e.g. To Center" value={formData.headsign} onChange={e => setFormData({...formData, headsign: e.target.value})} required /></div><div><label className="text-[8px] font-black uppercase mb-1 block text-zinc-400">Path ID</label><div className="relative"><input className="hig-input text-[11px] font-mono py-1.5 pr-8 uppercase" value={formData.shape_id} onChange={e => setFormData({...formData, shape_id: e.target.value})} required /><div className="absolute right-2.5 top-2 text-[7px] bg-black/5 px-1 rounded font-black opacity-40">ID</div></div></div><div className="p-3 bg-zinc-50 rounded-xl border border-zinc-100"><div className="flex items-center gap-2 mb-1 text-zinc-400"><Navigation size={12}/><span className="text-[8px] font-black uppercase">Geometry</span></div><div className="text-[10px] font-bold text-zinc-600 leading-relaxed">{activePoints.length > 0 ? `${activePoints.length} vertices found.` : 'No geometry found.'}</div></div>{selectedTrip.id && (<button type="button" onClick={() => { if(window.confirm('Dissolve mapping?')) api.delete(`/trips/${selectedTrip.id}`).then(fetchInitialData).then(() => setSelectedTrip(null)); }} className="w-full py-2 text-[8px] font-black text-red-400 hover:text-red-600 uppercase tracking-widest">Delete Record</button>)}</form></div>                            <div className="p-4 bg-white/50 backdrop-blur-md border-t border-zinc-100 rounded-b-[1.5rem] sticky bottom-0 flex justify-center">
                                <button 
                                    onClick={handleSave} 
                                    disabled={!isDirty} 
