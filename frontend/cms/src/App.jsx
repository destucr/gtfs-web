  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <header className="mb-12">
        <h1 className="text-4xl font-extrabold tracking-tight text-black mb-2">GTFS Control Center</h1>
        <p className="text-xl text-system-gray font-medium">Welcome back. Manage your transit network with precision.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {stats.map(s => (
          <Link key={s.path} to={s.path} className="hig-card p-6 flex flex-col group hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer">
            <div className={`w-12 h-12 ${s.color} rounded-xl flex items-center justify-center text-white mb-4 shadow-lg shadow-${s.color.split('-')[1]}-500/20`}>
              <s.icon size={24} />
            </div>
            ...
            <div className="flex items-center text-system-gray text-sm font-medium mt-auto group-hover:text-system-blue transition-colors">
              Manage Collection <ArrowRight size={14} className="ml-1" />
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="hig-card p-8 bg-gradient-to-br from-system-blue to-blue-600 text-white relative overflow-hidden group shadow-2xl shadow-system-blue/30">
        <div className="relative z-10">
          <h2 className="text-2xl font-bold mb-2">New: Route Studio Unified Editor</h2>
          <p className="text-white/80 max-w-md mb-6 font-medium">Design paths, snap to road networks, and assign stops in a single high-performance workspace.</p>
